...
class cRole : cStandart() {
    ...
    override fun postAdd(id: Int, hmColumnData: Map<iColumn, iData>, hmOut: MutableMap<String, Any>): String? {
        ...
        val rs = conn.executeQuery(" SELECT id FROM SYSTEM_permission ")
        while (rs.next()) alPerm.add(rs.getInt(1))
        rs.close()

        for (permID in alPerm) {
            val nextRolePermID = conn.getNextIntId("SYSTEM_role_permission", "id")
            conn.executeUpdate(" INSERT INTO SYSTEM_role_permission ( id, role_id, permission_id, permission_value ) VALUES ( $nextRolePermID , $id , $permID , 0 ) ")
        }
        return postURL
    }
}
