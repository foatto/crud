...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        fun getParamURL( aAlias: String, aAction: String, aRefererID: String?, aID: Int?,
                         aParentData: Map<String, Int>?, aParentUserID: Int?, aAltParams: String? ): String {
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    protected fun getSQLString( isForTable: Boolean, id: Int, alColumnList: List<iColumn>,
                                alSortColumn: List<iColumn>?, alSortDirect: List<String>?, alFindWord: List<String>? ): String {
        ...
        hsSelectTables.add( StringBuilder( model.tableName ).append(
...
//--- table part ----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    open fun getTable( hmOut: MutableMap<String, Any> ): TableResponse {
        ...
        val sort = try { hmParam[ AppParameter.SORT ]!!.toInt() } catch(t: Throwable ) { 0 }
        ...
        val pageNo = if( aliasConfig.pageSize > 0 ) try { hmParam[ AppParameter.PAGE ]!!.toInt() } catch(t: Throwable ) { 0 } else 0
        ...
        if( selectorParam != null ) {
            ...
            if( selectorParam.selectID != 0 &&                      // если задан parentID в виде selectID
                getParentID( aliasConfig.alias ) != null ) {   // иерархическая таблица ? ( иерархическая таблица всегда имеет себя в парентах )
        ...
        val selectorCancelURL = if( selectorParam != null ) {
                                    ...
                                    getParamURL( selectorParam.formAlias, AppAction.FORM, selectorParam.refererID, selectorParam.recordID, selectorParam.hmParentData,
    ...
    protected fun getTableRow( hmColumnData: Map<iColumn, iData>, dataRowNo: Int, tableRowStart: Int, selectorID: String?, selectorParam: SelectorParameter?, refererID: String,
                               alTableCell: MutableList<TableCell>, alTableRowData: MutableList<TableRowData>, hmOut: MutableMap<String, Any> ): Int {
        ...
        for( gi in 0 until model.alTableGroupColumn.size ) {
            ...
            if( curValue != arrCurGroupValue!![ gi ] ) {
                ...
                //--- наименование группы
                alTableCell.add( setTableGroupColumnStyle( hmColumnData, curGroupColumn, TableCell(
                    aRow = row,
                    aCol = col,
                    aRowSpan = 1,
    ...
    protected open fun getTableRowSelectButton( row: Int, col: Int, selectorParam: SelectorParameter, hmColumnData: Map<iColumn, iData>, hmOut: MutableMap<String, Any> ): TableCell {
        ...
        return TableCell(
            ...
            aText = "",
            aUrl = getParamURL( selectorParam.formAlias, AppAction.FORM, selectorParam.refererID, selectorParam.recordID, selectorParam.hmParentData,
                                selectorParam.parentUserID, "&${AppParameter.FORM_DATA}=$formDataID" ),
            aInNewWindow = false
        )
    }
    ...
    protected open fun getTableRowGoto( selectorID: String?, hmColumnData: Map<iColumn, iData>, indexChild: Int, alPopupData: MutableList<TablePopupData> ): String? {
        ...
        val popupURL = getParamURL( childAlias, childData.action, null, if( childData.action == AppAction.FORM ) 0 else null, hmNewParentData, newParentUserID,
    ...
    protected open fun getServerAction(): MutableList<ServerActionButton> { return mutableListOf() }

    protected open fun getClientAction(): MutableList<ClientActionButton> { return mutableListOf() }
...
//--- form part -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    protected fun getFormSelector( formParam: String, forClear: Boolean, id: Int, refererID: String?, column: iColumn, hmColumnData: Map<iColumn, iData>,
                                   hmOut: MutableMap<String, Any> ): String {
...
//--- find part -----------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    fun doFind( findStr: String, hmOut: MutableMap<String, Any> ): String? {
        ...
        if( finderID == null || refererID == null || refererURL == null ) return null
    ...
    protected fun checkInput(id: Int, alFormData: List<FormData>, alColumnList: List<iColumn>, hmColumnData: MutableMap<iColumn, iData>, hmOut: MutableMap<String, Any>): String? {
        ...
        if( selectorID != 0 ) {
            ...
            if( selectorParam.forClear ) {
                ...
                return getParamURL( selectorParam.formAlias, AppAction.FORM, selectorParam.refererID, selectorParam.recordID, selectorParam.hmParentData, selectorParam.parentUserID,
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    protected fun deleteIsReaded( id: Int, deleteAll: Boolean ) {
        if( aliasConfig.isNewable ) {
            stm.executeUpdate( " DELETE FROM SYSTEM_new WHERE table_name = '${model.tableName}' AND row_id = $id " +
