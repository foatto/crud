...
import foatto.sql.SpringConnection
...
abstract class CoreSpringController( val jdbcTemplate: JdbcTemplate ) {
...
//    @Transactional
    open fun app(
        ...
        //--- желательно не переиспользовать SpringConnection между вызовами,
        //--- чтобы оставшиеся нестёртыми sql-выражения после последних транзакций (неважно, закомиченные иои откаченные) не записались повторно в реплики
        val conn = SpringConnection( jdbcTemplate, CoreSpringApp.dbConfig )
