...
object PriceData {
    ...
    //--- загрузка прайса
    fun loadPrice(stm: CoreAdvancedStatement, priceType: Int ): Map<Int, List<Pair<Int, Double>>> {
        val hmPrice = mutableMapOf<Int, MutableList<Pair<Int, Double>>>()

        val rs = stm.executeQuery( " SELECT catalog_id , ye , mo , da , price_value FROM SHOP_price WHERE price_type = $priceType ORDER BY catalog_id , ye , mo , da " )
        while( rs.next() ) {
            ...
            var alHistory = hmPrice[ catalogID ]
            if( alHistory == null ) {
                alHistory = mutableListOf()
                hmPrice[ catalogID ] = alHistory
            }
