            ...
            alWorkerId.forEach { sWorkerId ->
                ...
                for (day in 1..maxDaysInMonth) {
                    val workHour = when (LocalDate.of(curYear, curMonth, day).dayOfWeek.value) {
                        in 1..5 -> (application as iShopApplication).workHourInWorkDay?.toDoubleOrNull() ?: 8.0
                        6 -> (application as iShopApplication).workHourInSaturday?.toDoubleOrNull() ?: 6.0
                        else -> 0.0
                    }
                    ...
                    if (workerId == currentWorkerId && day == curDay) {
                        ...
                        stm.executeUpdate(
                            """
                                INSERT INTO SHOP_work_hour ( id , worker_id , ye , mo , da , work_hour )
                                VALUES ( 
                                    ${stm.getNextIntId("SHOP_work_hour", "id")} ,
                                    $workerId , $curYear , $curMonth , $day , $workHour
