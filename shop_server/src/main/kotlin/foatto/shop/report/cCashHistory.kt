...
class cCashHistory : cAbstractReport() {
    ...
    override fun postReport( sheet: WritableSheet ) {
        ...
        offsY++
        sheet.addCell( Label( 1, offsY++, "с ${if( reportBegDay < 10 ) "0" else ""}$reportBegDay.${if( reportBegMonth < 10 ) "0" else ""}$reportBegMonth.$reportBegYear" +
    ...
    fun calcOut( aWarehouseID: Int, arrDT: IntArray ): Double {
        ...
        //--- для всех продаж
        var rs = stm.executeQuery( " SELECT id , discount FROM SHOP_doc WHERE is_deleted = 0 AND sour_id = $aWarehouseID AND doc_type = ${DocumentTypeConfig.TYPE_OUT} " +
        ...
        rs = stm.executeQuery( " SELECT id , discount FROM SHOP_doc WHERE is_deleted = 0 AND dest_id = $aWarehouseID AND doc_type = ${DocumentTypeConfig.TYPE_RETURN_OUT} " +
                               " AND doc_ye = ${arrDT[ 0 ]} AND doc_mo = ${arrDT[ 1 ]} AND doc_da = ${arrDT[ 2 ]} " )
        while( rs.next() ) {
            alDocID.add( rs.getInt( 1 ) )
