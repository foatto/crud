    ...
    protected fun outItem(sheet: WritableSheet, aOffsY: Int, ci: CalcItem, whDest: Int): Int {
        ...
        if (ci.ii.isFolder) {
            sheet.addCell(
                Label(
                    2,
                    offsY,
                    if (userConfig.upIsUseThousandsDivider) {
                        getSplittedLong(ci.subItemCount.toLong())
                    } else {
                        ci.subItemCount.toString()
                    },
                    wcfCellCBStdYellow
                )
            )
        ...
        for ((wName, wID) in tmWarehouseID) {
            ...
            sheet.addCell(
                Label(
                    offsX++,
                    offsY,
                    getSplittedDouble(count, -1, userConfig.upIsUseThousandsDivider, userConfig.upDecimalDivider),
                    if (ci.ii.isFolder) {
                        if (count < 0) {
                            wcfCellCBStdRed
                        } else {
        ...
        if (whDest == 0)
            sheet.addCell(
                Label(
                    offsX++,
                    offsY,
                    getSplittedDouble(whSum, -1, userConfig.upIsUseThousandsDivider, userConfig.upDecimalDivider),
                    if (ci.ii.isFolder) {
                        if (whSum < 0) {
                            wcfCellCBStdRed
                        } else {
        ...
        sheet.addCell(
            Label(
                offsX++,
                offsY,
                getSplittedDouble(priceSum, 2, userConfig.upIsUseThousandsDivider, userConfig.upDecimalDivider),
                if (ci.ii.isFolder) {
                    if (priceSum < 0) {
                        wcfCellRBStdRed
                    } else {
        ...
        if (ci.tmSubItem != null) {
            ...
            ci.tmSubItem!!.keys.forEach { name ->
                val ciSub = ci.tmSubItem!![name]!!
                if (ciSub.tmSubItem == null) {
                    offsY = outItem(sheet, offsY, ciSub, whDest)
                }
            }
            ci.tmSubItem!!.keys.forEach { name ->
                val ciSub = ci.tmSubItem!![name]!!
