...
//  doc_id            INT,            -- учитывается при загрузке
    ...
    companion object {
        fun loadDocumentContentData(stm: CoreAdvancedStatement, docID: Int ): ArrayList<DocumentContentData> {
            val alDCD = ArrayList<DocumentContentData>()

            val rs = stm.executeQuery( " SELECT id , sour_id , dest_id , sour_num , dest_num FROM SHOP_doc_content WHERE doc_id = $docID AND is_deleted = 0 ORDER BY id " )
            while( rs.next() ) {
                var p = 1
