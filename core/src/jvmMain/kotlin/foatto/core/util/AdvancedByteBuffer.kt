...
//--- конструкторы --------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    constructor( capacity: Int, byteOrder: ByteOrder = ByteOrder.BIG_ENDIAN ) {
        buffer = ByteBuffer.allocate( Math.max( capacity, 1 ) )
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    fun skip( count: Int ): AdvancedByteBuffer {
        for( i in 0 until count ) buffer.get()
        return this
    }
    ...
    fun getInt3(): Int {
        ...
        return if( buffer.order() == ByteOrder.BIG_ENDIAN ) ( b0 and 0xFF shl 16 ) or ( b1 and 0xFF shl 8 ) or ( b2 and 0xFF )
                                                       else ( b2 and 0xFF shl 16 ) or ( b1 and 0xFF shl 8 ) or ( b0 and 0xFF )
    }
    ...
    fun getShortString(): String {
        ...
        for( i in 0 until len ) sb.append( buffer.char )
        return sb.toString()
    }
...
//!!! сделать красиво итератором по строке
        for( i in 0 until len ) buffer.putChar( s[ i ] )
        return this
    }
...
//!!! сделать красиво итератором по строке
    ...
    fun getHex( aSb: StringBuilder?, withSpace: Boolean ): StringBuilder {
        ...
        while( buffer.hasRemaining() ) byteToHex( buffer.get(), sb, withSpace )
        return sb
    }
    ...
    fun putHex( hex: CharSequence? ): AdvancedByteBuffer {
        ...
        if( hex != null ) {
            var i = 0
            while( i < hex.length ) buffer.put( hexToByte( hex[ i++ ], hex[ i++ ] ) )
        }
        return this
    }
