...
class ObjectSelector {
    ...
    fun fillColumns( model: mAbstract, isRequired: Boolean, isSelector: Boolean,
        ...
        if( isSelector ) {
            columnObjectName.selectorAlias = if( selectorAlias == null ) "mms_object" else selectorAlias
            //--- только для "родного" алиаса
            if( selectorAlias == null ) columnObjectName.addSelectorColumn( columnObject, columnObjectID )
            else columnObjectName.addSelectorColumn( selectorColumnTo!!, selectorColumnFrom!! )
