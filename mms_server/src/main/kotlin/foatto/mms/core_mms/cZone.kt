...
class cZone : cStandart() {
    ...
    override fun getNextID(hmColumnData: Map<iColumn, iData>) = conn.getNextIntId(arrayOf("MMS_object", "MMS_zone"), arrayOf("id", "id"))
    ...
    override fun postDelete(id: Int, hmColumnData: Map<iColumn, iData>) {
        ...
        conn.executeUpdate(" DELETE FROM XY_point WHERE element_id IN ( SELECT id FROM XY_element WHERE object_id = $id ) ")
        conn.executeUpdate(" DELETE FROM XY_property WHERE element_id IN ( SELECT id FROM XY_element WHERE object_id = $id ) ")
        conn.executeUpdate(" DELETE FROM XY_element WHERE object_id = $id ")
    }
    ...
    private fun updateZoneInfo(id: Int, zoneInfo: String) {
        ...
        if (elementId != 0) {
            if (conn.executeUpdate(
                    " UPDATE XY_property SET property_value = '$zoneInfo' WHERE element_id = $elementId AND property_name = '${XyProperty.TOOL_TIP_TEXT}' "
                ) == 0
            ) {
