...
abstract class cShowAbstractObject : cAbstractForm() {
    ...
    companion object {
        val ZONE_SHOW_NONE = 0    // не показывать никаких зон
        val ZONE_SHOW_ACTUAL = 1    // показывать только актуальные/используемые
        val ZONE_SHOW_ALL = 2    // показывать все зоны
    }
    ...
    protected fun getShowURL( hmOut: MutableMap<String, Any>, sd: XyStartData ): String {
        ...
        for( objectIndex in sd.alStartObjectData.indices ) {
            ...
            val oc = ObjectConfig.getObjectConfig( stm, userConfig, sod.objectID )
        ...
        if( showZoneType != ZONE_SHOW_NONE ) {
            ...
            for( objectIndex in sd.alStartObjectData.indices ) {
                ...
                val hmZoneLimit = ZoneLimitData.getZoneLimit( stm, userConfig, hmZoneData, objectID, /*waybillID,
                    StringFunction.Arr_DateTime( sd.arrBegDT ), StringFunction.Arr_DateTime( sd.arrEndDT ),*/ 0 )
        ...
        for( zoneID in hmActualZone.keys ) sd.alStartObjectData.add( XyStartObjectData( zoneID, "mms_zone", true, false, hmActualZone[ zoneID ]!! ) )
