...
class GalileoNettyHandler(
    ...
    override fun channelRead(ctx: ChannelHandlerContext, msg: Any) {
        val bbIn = msg as ByteBuf
        var conn: CoreAdvancedConnection? = null

        try {
            if (packetHeader.toInt() == 0) {
                if (bbIn.readableBytes() < 1 + 2) {
                    return
                }
                packetHeader = bbIn.readByte()
                packetSize = bbIn.readShortLE().toInt() and 0xFFFF
                ...
                if (packetHeader.toInt() != 0x01) {
                    ...
                    MMSTelematicFunction.writeError(
                        ...
                        fwVersion = fwVersion,
                        begTime = begTime,
                        address = ctx.channel().localAddress().toString(),
                        status = status,
                    ...
                    throw Throwable("Wrong packet header = $packetHeader")
                }
            }

            //--- ждём основные данные + 2 байта CRC
            if (bbIn.readableBytes() < packetSize + 2) {
                return
            }
            ...
            while (bbIn.readableBytes() > 2) {
                AdvancedLogger.debug("readableBytes = ${bbIn.readableBytes()}")

                //--- тег данных
                val tag = bbIn.readByte().toInt() and 0xFF
                ...
                when (tag) {
                    ...
                    0x01 -> {
                        val hwVersion = bbIn.readByte().toInt() and 0xFF // хрен знает, что с ней делать
                        AdvancedLogger.debug("hardware version = $hwVersion")
                    }

                    //--- версия прошивки
                    0x02 -> {
                        fwVersion = (bbIn.readByte().toInt() and 0xFF).toString()
                    ...
                    0x30 -> {
                        ...
                        wgsY = bbIn.readIntLE() * 10
                        wgsX = bbIn.readIntLE() * 10
                    }

                    //--- speed & angle
                    0x33 -> {
                        speed = roundSpeed((bbIn.readShortLE().toInt() and 0xFFFF) / 10.0)
                        bbIn.readShortLE()    // SKIP angle
                    }

                    //--- altitude
                    0x34 -> bbIn.readShortLE()    // SKIP altitude

                    //--- HDOP
                    0x35 -> bbIn.readByte() // SKIP HDOP

                    //--- device status - из всех статусов нам пока интересен только статус парковки по уровню вибрации
                    0x40 -> isParking = bbIn.readShortLE().toInt() and 0x0001 == 0

                    //--- power voltage
                    0x41 -> powerVoltage = bbIn.readShortLE().toInt() and 0xFFFF

                    //--- accum voltage
                    0x42 -> accumVoltage = bbIn.readShortLE().toInt() and 0xFFFF

                    //--- controller temperature
                    0x43 -> controllerTemperature = bbIn.readByte().toInt()

                    //--- acceleration
                    0x44 -> bbIn.readIntLE()

                    //--- out status
                    0x45 -> bbIn.readShortLE()

                    //--- in status - не совсем понятно - дискретных входов как бы нет, а приходит аж 16 бит
                    0x46 -> bbIn.readShortLE()// & 0xFFFF;

                    //--- EcoDrive
                    0x47 -> bbIn.readIntLE()  // SKIP EcoDrive

                    //--- Расширенный статус терминала
                    0x48 -> bbIn.readShortLE()

                    //--- in voltage / impulse count / impulse frequency
                    in 0x50..0x57 -> tmUniversalSensor[tag - 0x50] = bbIn.readShortLE().toInt() and 0xFFFF

                    //--- RS-232
                    0x58, 0x59 -> bbIn.readShortLE()

                    //--- показатель счётчика электроэнергии РЭП-500
                    0x5A -> bbIn.readIntLE()  // SKIP
                    ...
                    0x5B -> {
                        ...
                        throw Throwable("serialNo = $serialNo\n unsupported tag = 0x${tag.toString(16)}\n disable refrigerator data, please")
                    }
                    ...
                    in 0x60..0x62 -> tmRS485Fuel[tag - 0x60] = bbIn.readShortLE().toInt() and 0xFFFF
                    ...
                    in 0x63..0x6F -> {
                        tmRS485Fuel[tag - 0x60] = bbIn.readShortLE().toInt() and 0xFFFF
                        tmRS485Temp[tag - 0x60] = bbIn.readByte().toInt()
                    }

                    //--- thermometer
                    in 0x70..0x77 -> {
                        bbIn.readByte()  //.toInt() and 0xFF - thermometerID
                        bbIn.readByte()  //.toInt() - value
                    }
                    ...
                    in 0x80..0x87 -> {
                        bbIn.readByte()  //.toInt() and 0xFF - DS_ID
                        bbIn.readByte()  //.toInt() - temp
                        bbIn.readByte()  //.toInt() and 0xFF) * 100 / 255 - humidity
                    }
                    ...
                    in 0x88..0x89 -> bbIn.readByte()

                    //--- RS-485 основные/типовые (0..2) - показания температуры
                    0x8A, 0x8B, 0x8C -> tmRS485Temp[tag - 0x8A] = bbIn.readByte().toInt()

                    //--- iButton 0
                    0x90 -> bbIn.readIntLE()

                    //--- CAN8BITR16..CAN8BITR31
                    in 0xA0..0xAF -> bbIn.readByte()

                    //--- CAN16BITR6..CAN16BITR15
                    in 0xB0..0xB9 -> bbIn.readShortLE()

                    //--- FMS-Standart: fuel
                    0xC0 -> bbIn.readIntLE()   // / 2).toLong()   //!!! не учитывается, что беззнаковое целое

                    //--- CAN
                    0xC1 -> {
                        canFuelLevel = ((bbIn.readByte().toInt() and 0xFF) * 0.4f).roundToInt()
                        canCoolantTemperature = (bbIn.readByte().toInt() and 0xFF) - 40
                        canEngineRPM = ((bbIn.readShortLE().toInt() and 0xFFFF) * 0.125f).roundToInt()
                    }

                    //--- FMS-Standart: run
                    0xC2 -> bbIn.readIntLE()   // * 5L    //!!! не учитывается, что беззнаковое целое

                    //--- CAN_B1
                    0xC3 -> bbIn.readIntLE()

                    //--- CAN8BITR0..7 или CAN-LOG, зависит от настроек
                    in 0xC4..0xD2 -> bbIn.readByte()

                    //--- iButton 1
                    0xD3 -> bbIn.readIntLE()

                    //--- absolute/summary run - не учитывается, что беззнаковое целое, однако машине больше 2 млн. км всё равно не пробежать
                    0xD4 -> absoluteRun = bbIn.readIntLE()

                    //--- iButton status
                    0xD5 -> bbIn.readByte()  //.toInt() and 0xFF

                    //--- CAN16BITR0..4 или CAN-LOG, зависит от настроек
                    0xD6, 0xD7, 0xD8, 0xD9, 0xDA -> bbIn.readShortLE()

                    //--- CAN32BITR0..4 или CAN-LOG, зависит от настроек
                    0xDB, 0xDC, 0xDD, 0xDE, 0xDF -> bbIn.readIntLE()
                    ...
                    //--- ответ на команду
                    0xE1 -> {
                        val answerLen = bbIn.readByte().toInt() and 0xFF
                        val arrAnswer = ByteArray(answerLen)
                        bbIn.readBytes(arrAnswer)
                    ...
                    0xEA -> {
                        ...
                        if (userDataType == 0x02) {
                            ...
                            bbIn.skipBytes(userDataSize - 1)
                        } else if (userDataType == 0x03) {
                            val dataVersion = bbIn.readByte().toInt() and 0xFF
                            when (dataVersion) {
                                2 -> {
                            ...
                            for (groupIndex in 0..3) {
                                val esdStatus = bbIn.readByte().toInt() and 0xFF
                                ...
                                if (esdStatus != 0) {
                                    tmESDVolume[groupIndex] = bbIn.readIntLE() * 0.01                      // litres
                                    tmESDFlow[groupIndex] = bbIn.readIntLE() * 0.1                         // litres/hour
                                    tmESDCameraVolume[groupIndex] = bbIn.readIntLE() * 0.01                // litres
                                    tmESDCameraFlow[groupIndex] = bbIn.readIntLE() * 0.1                   // litres/hour
                                    tmESDCameraTemperature[groupIndex] = bbIn.readByte().toInt()         // C
                                    tmESDReverseCameraVolume[groupIndex] = bbIn.readIntLE() * 0.01         // litres
                                    tmESDReverseCameraFlow[groupIndex] = bbIn.readIntLE() * 0.1            // litres/hour
                                    tmESDReverseCameraTemperature[groupIndex] = bbIn.readByte().toInt()  // C
                                }
                            }
                        }
                    ...
                    in 0xF0..0xF9 -> bbIn.readIntLE()

                    //--- Расширенные теги
                    0xFE -> {
                        var extTagDataLen = bbIn.readShortLE().toInt() and 0xFFFF
                        AdvancedLogger.debug("serialNo = $serialNo\n start extTagDataLen = $extTagDataLen")
                        while (extTagDataLen > 0) {
                            val extTag = bbIn.readShortLE().toInt() and 0xFFFF
                            extTagDataLen -= 2
            ...
            val crc = bbIn.readShortLE()

            AdvancedLogger.debug("remaining = " + bbIn.readableBytes())
            status += " DataRead;"
            ...
            deviceConfig?.let { dc ->
                val (cmdID, cmdStr) = MMSTelematicFunction.getCommand(conn, dc.deviceId)
                ...
                if (cmdStr != null) {
                    ...
                    if (cmdStr.isNotEmpty()) {
                        ...
                        val bbOut = ctx.alloc().buffer(256) // 256 с запасом

                        bbOut.writeByte(0x01)
                        bbOut.writeShortLE(dataSize)

                        bbOut.writeByte(0x03)
                        bbOut.writeBytes(arrIMEI)

                        bbOut.writeByte(0x04)
                        bbOut.writeShortLE(terminalId)

                        bbOut.writeByte(0xE0)
                        bbOut.writeIntLE(0)

                        bbOut.writeByte(0xE1)
                        bbOut.writeByte(cmdStr.length)
                        bbOut.writeBytes(cmdStr.toByteArray())

                        //--- кто бы мог подумать: CRC отправляется в big-endian, хотя сами данные приходят в little-endian
                        //!!! проверить варианты с writeShortLE и/или crc16_modbus с параметром isBigEndianOut = false/true
                        bbOut.writeShortLE(crc16_modbus(bbOut.array(), bbOut.arrayOffset(), dataSize + 3, true))

                        ctx.write(bbOut)
                    }
                    //--- отметим успешную отправку команды
                    MMSTelematicFunction.setCommandSended(conn, cmdID)
                    status += " CommandSend;"
                }
            }
            //--- common flush without (!) closing to continuous connection
            ctx.flush()
            ...
            deviceConfig?.let { dc ->
                MMSTelematicFunction.writeSession(
                    ...
                    fwVersion = fwVersion,
                    begTime = begTime,
                    address = ctx.channel().remoteAddress().toString() + " -> " + ctx.channel().localAddress().toString(),
                    status = status,
                    errorText = errorText,
                    dataCount = dataCount,
                    dataCountAll = dataCountAll,
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    private fun checkConnection(conn: CoreAdvancedConnection?) = conn ?: AdvancedConnection(dbConfig)
    ...
    private fun sendAccept(ctx: ChannelHandlerContext, crc: Short) {
        val bbOut = ctx.alloc().buffer(3)

        bbOut.writeByte(0x02)
